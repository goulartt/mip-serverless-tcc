...
public class GetSurveyByIdRequest implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
	...
	public APIGatewayProxyResponseEvent handleRequest(APIGatewayProxyRequestEvent input, Context context) {
		...
		Survey resultList = null;
		try (Session session = sessionFactory.openSession()) {
			session.beginTransaction();

			resultList = (Survey) session.load(Survey.class, Long.valueOf(input.getQueryStringParameters().get("id")));
